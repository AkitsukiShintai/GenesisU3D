...
public class TechnicParameterPanel : BasePanel, IDragHandler, IBeginDragHandler, IEndDragHandler
{
    ...
    void RushALL()
    {
        ...
        for (int i = 0; i < aboutShoe.ParameterList.Count; i++)
        {
            ...
            if (aboutShoe.ParameterList[i].ShowPosX != -1)
                continue;
            if (aboutShoe.ParameterList[i].ShowInterfacePart == ParameterBase.ShowInterfacePositon.Technic &&aboutShoe.ParameterList[i].IsShowInEdit)
            {
    ...
    private void RushDetail()
    {
        ...
        for (int i = 0; i < aboutShoe.ParameterList.Count; i++)
        {
            ...
            if (aboutShoe.ParameterList[i].ShowPosX != -1)
                continue;
            if (aboutShoe.ParameterList[i].ShowInterfacePart == ParameterBase.ShowInterfacePositon.TechnicDetail&&aboutShoe.ParameterList[i].IsShowInEdit)
            {
               ...
               && !((ParameterDouble)aboutShoe.ParameterList[i]).IsComboxDoubleUnion)
                ...
                else if (aboutShoe.ParameterList[i].ValueType == ParameterBase.eValueType.Color4)
                {
                    ...
                    GameObject go = AllUIManager.Instance.GetUIObject(UIObjectName.ColorSelect);
                    go.GetComponent<ColorSelect>().Init(aboutShoe.ParameterList[i], aboutShoe);
                    go.transform.SetParent(Contant.transform);
                    //根据子物体高度重新设置Contant位置
                    AllHeightOfChildren += go.GetComponent<RectTransform>().sizeDelta.y;
            ...
            //if (!IsMet && aboutShoe.ParameterList[i].IsAboutMet)
            //continue;
            //if (IsMet && !aboutShoe.ParameterList[i].IsAboutMet)
            //continue;
    ...
    //面板中的形态调整和细节调整按钮点击事件
    public void FormationAdjustButtonClick()
    {
        RushALL();
    }

    public void DetailAdjustButtonClick()
    {
        RushDetail();
    }
