...
public class ColorMatchPanel : BasePanel {
    ...
    private void ShowUIs()
    {
        ...
        //TODO
        foreach (Page2Data t in UISeletedData.Instance.PageDatas.PageDatas[4].dPage2)
        {
            for (int j = 0; j < t.FabricParameterList.Count; j++)
            {
                GameObject go = AllUIManager.Instance.GetUIObject(UIObjectName.LeftButton);
                go.AddComponent<ParameterButton>().Init(t, t.FabricParameterList[j]);
